...
Do not run the evaluation before `extract_lmms_eval_samples` returns a real existing `samples_jsonl_abs`.
...
4. On inference success, call `extract_lmms_eval_samples` with the inference `log_path`, LMMS Eval cwd, and task name.
5. Start OmniDocBench evaluation with `start_long_command`, passing `next_skill_input.prediction_jsonl` from `extract_lmms_eval_samples`.
...
8. On evaluation success, call `extract_omnidocbench_metrics` with the evaluation `log_path`, OmniDocBench cwd, and `job.outputs.report_path`.
9. Finish only after `extract_omnidocbench_metrics` returns all required metrics and the Markdown report path.

## Resume Rules

If state/history shows a long command is still running, use `inspect_long_command` before starting a new command.

If inference already succeeded but the JSONL was not extracted, call `extract_lmms_eval_samples` from the recorded inference `log_path`.

If a prediction JSONL already exists in state, skip rerunning inference and continue to OmniDocBench evaluation.

If OmniDocBench already succeeded but metrics were not extracted, call `extract_omnidocbench_metrics` from the recorded evaluation `log_path`.
...
- `extract_lmms_eval_samples` returned an existing prediction JSONL.
- OmniDocBench evaluation command succeeded.
- `extract_omnidocbench_metrics` returned `text_block_Edit_dist`, `display_formula_CDM`, `table_TEDS`, `table_TEDS_structure_only`, `reading_order_Edit_dist`, and `overall_notebook`.
